...
<div class="col-lg-12">
    <div class="card m-b-30 mt-5">
        <CardHeader title="Edit Vendor" btnRoute="vendors" btnName="Vendors" />
        <div class="card-body">
            <vue-loaders-ball-clip-rotate-multiple v-if="loading" scale="1.5" color="#3f51b5" />
            <form @submit.prevent="updateVendor($event)" v-bind:class="{ lessOpacity: loading }">
                <div class="row">
                    <div class="col-lg-4">
                    ...
                    <div class="col-lg-4">
                        ...
                        " v-model="vendor.status">
                            ...
                            <p class="text-danger" v-if="
                                errors.status.length > 0 && !vendor.status
...
export default {
    ...
    methods: {
        ...
        async getVendorCities() {
            this.cities = await this.$root.getAllData(
                `/api/user/districts/showByCountrys/${this.vendor.country}`
            );
            this.getAllVendorThanas();
        },
        async getAllVendorThanas() {
            this.thanas = await this.$root.getAllData(
                `/api/user/thanas/showByDistricts/${this.vendor.city}`
            );
        },
        ...
        async getFaq() {
            ...
            let data = await this.$root.getAllData(
                `/api/user/vendors/${this.$route.params.id}`
            );
            this.vendor = data[0];
            this.loading = false;
        },
        ...
        updateVendor(event) {
            this.$root.updateData(this.vendor, `/api/user/vendors/${this.$route.params.id}`).then(response => {
                this.$fire({
