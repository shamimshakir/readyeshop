...
<div class="col-lg-12">
    <div class="card m-b-30 mt-5">
        <CardHeader title="Edit Setup" btnRoute="sms_setups" btnName="SMS Setups" />
        <div class="card-body">
            <vue-loaders-ball-clip-rotate-multiple v-if="loading" scale="1.5" color="#3f51b5" />
            <form @submit.prevent="updateSetup($event)" v-bind:class="{ lessOpacity: loading }">
                <div class="row">
                    <div class="col-lg-6">
    ...
    methods: {
        ...
        async getSetupInfo() {
            ...
            let data = await this.$root.getAllData(
                `/api/user/smssetups/${this.$route.params.id}`
            );
            this.setup = data[0];
            this.loading = false;
        },
        ...
        updateSetup (event) {
            this.$root.updateData(this.setup, `/api/user/smssetups/${this.$route.params.id}`).then(response => {
                this.$fire({
