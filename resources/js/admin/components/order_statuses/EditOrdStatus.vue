...
<div class="col-lg-12">
    <div class="card m-b-30 mt-5">
        <CardHeader title="Edit Status" btnRoute="ord_statuses" btnName="Order Status" />
        <div class="card-body">
            <form @submit.prevent="updateOdSts($event)">
...
export default {
    ...
    methods: {
        ...
        async getodSts() {
            this.loading = true;
            this.odSts = await this.$root.getAllData(
                `/api/user/orderStatus/${this.$route.params.id}`
            );
            this.loading = false;
        },
        updateOdSts(event) {
            this.$root.updateData(this.odSts, `/api/user/orderStatus/${this.$route.params.id}`).then(response => {
