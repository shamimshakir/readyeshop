...
<div class="col-lg-12">
    <div class="card m-b-30 mt-5">
        <CardHeader title="Edit News" btnRoute="news" btnName="News" />
        <div class="card-body">
            <vue-loaders-ball-clip-rotate-multiple v-if="loading" scale="1.5" color="#3f51b5" />
            <form @submit.prevent="updateNews($event)" v-bind:class="{ lessOpacity: loading }">
                <div class="row">
                    <div class="col-lg-6">
    ...
    methods: {
        ...
        async getNews() {
            ...
            let data = await this.$root.getAllDataWithImages(
                `/api/user/news/${this.$route.params.id}`
            );
            this.news_image_url = data[0].url;
            this.sNews = data[0].data[0];
            this.loading = false;
        },
        updateNews(event) {
            ...
            this.axios
                .post(`${this.$store.state.hostBase}/api/user/news/${this.$route.params.id}?_method=PUT`, form, {
                    headers: {
