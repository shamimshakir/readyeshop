...
<div class="col-lg-12">
    <div class="card m-b-30 mt-5">
        <CardHeader title="Edit tpauth" btnRoute="third_party_auths" btnName="tpauths" />
        <div class="card-body">
            <vue-loaders-ball-clip-rotate-multiple v-if="loading" scale="1.5" color="#3f51b5" />
            <form @submit.prevent="updatetpauth($event)" v-bind:class="{ lessOpacity: loading }">
                                <div class="row">
                    <div class="col-lg-12">
...
export default {
    ...
    methods: {
        ...
        async gettpauth() {
            this.loading = true;
            this.tpauth = await this.$root.getAllData(
                `/api/user/thirdpartyauths/${this.$route.params.id}`
            );
            this.loading = false;
        },

        updatetpauth(event) {
            this.$root.updateData(this.tpauth, `/api/user/thirdpartyauths/${this.$route.params.id}`).then(response => {
                this.$fire({
