...
<div class="col-lg-12">
    <div class="card m-b-30 mt-5">
        <CardHeader title="Edit Product Filter" btnRoute="productfilter" btnName="Product Filter" />
        <div class="card-body">
            <vue-loaders-ball-clip-rotate-multiple v-if="loading" scale="1.5" color="#3f51b5" />
            <form @submit.prevent="updateProductFilters($event)" v-bind:class="{ lessOpacity: loading }">
                <div class="row">
                    <div class="col-lg-6">
    ...
    methods: {
        ...
        async getProductFilter() {
            ...
            let data = await this.$root.getAllData(
                `/api/user/productFilter/${this.$route.params.id}`
            );
            console.log(data[0]);
            this.productfilters = data[0];
            this.allOptions=data[0].filteroptions;
            this.loading = false;
        },
        addOptionRow() {
            let obj = {
        ...
        updateProductFilters(event) {
            ...
            this.$root.updateData(this.productfilters, `/api/user/productFilter/${this.$route.params.id}`).then(response => {
