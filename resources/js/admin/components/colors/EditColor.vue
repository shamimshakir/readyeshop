...
<div class="col-lg-12">
    <div class="card m-b-30 mt-5">
        <CardHeader title="Edit Color" btnRoute="colors" btnName="Colors" />
        <div class="card-body">
            <form @submit.prevent="updateColor($event)">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" v-model="color.other_language">
                </div>
                <div class="form-group">
...
export default {
    ...
    methods: {
        ...
        async getColor() {
            this.loading = true;
            this.color = await this.$root.getAllData(
                `/api/user/colors/${this.$route.params.id}`
            );
            this.loading = false;
        },
        updateColor(event) {
            this.$root.updateData(this.color, `/api/user/colors/${this.$route.params.id}`).then(response => {
