        ...
        <div class="card m-b-30 mt-5">
            <div class="card-body">
                ...
                <div v-bind:class="{ lessOpacity: loading }">
                    <form @submit.prevent="informShipper($event)">
                        <div class="row">
                            <div class="col-md-12">
                                <label for="od_status">Shipper </label>
                                <select v-model="orderInfo.shipper_id" class='form-control'>
                                    <option value="">Select a Shipper</option>
                                    <option v-for="shipper in shippers" :value="shipper.id" :key="shipper.id">{{ shipper.sp_name }}</option>
                                </select>
                            </div>
                            <div class="col-md-12">
...
export default {
    ...
    methods: {
        informShipper(e){
            this.$root.addData(this.orderInfo, '/api/user/orders/shiper_imform').then(response => {
                this.$fire({
