        ...
        <div class="card-body">
            <div class="row">
                <div class="col-12">
                    ...
                    <form @submit.prevent="updateProductPurchase($event)">
                        <div class="row">
                            <div class="col-md-4">
                        ...
                        <div class="row mt-3">
                            <div class="col-lg-12">
                                <router-link to="/product_purchase" class="btn btn-secondary waves-effect">Back</router-link>
...
export default {
    ...
    methods: {
        ...
        async getProductPurchase(){
            let dataArr = await this.$root.getAllData(
                `/api/user/productreceives/${this.$route.params.id}`
            );
            let singleData = dataArr[0];
            this.ppurchase.comments = singleData.comments;
            this.ppurchase.client_id = singleData.vendor.id;
            this.ppurchase.invoice_date = singleData.invoice_date;
            singleData.trn_mat_receives.forEach(async (elem, index) => {
                let catProducts = await this.getProductByCategory(elem.categories.id);
                let productColors = catProducts.filter(catPro => catPro.id == elem.products.id).map(catPro => catPro.productcolors);
                let productSizes = catProducts.filter(catPro => catPro.id == elem.products.id).map(catPro => catPro.productsizes);
                this.rows.push({
                    id: index,
                    ...
                    selectedCategory: elem.categories ? elem.categories.id : "",
                    selectedProduct: elem.products ? elem.products.id : "",
                    unitPrice: elem.products.pd_price,
                    selectedQty: elem.qty,
                    selectedColor: elem.colors ? elem.colors.id : "",
                    selectedSize: elem.sizes ? elem.sizes.id : "",
                });
            });
        },
        getProdcutCost() {
        ...
        updateProductPurchase(e) {
            ...
            this.$root.updateData(ppurchaseFormData, `/api/user/productreceives/${this.$route.params.id}`).then(response => {
                this.$fire({
        ...
        async getProductByCategory(id) {
            return await this.$root.getAllData(`/api/user/products/showbycatagory/${id}`);
        },
