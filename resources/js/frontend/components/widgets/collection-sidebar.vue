                    ...
                    <div class="collection-collapse-block-content">
                        <div class="collection-brand-filter">
                            <ul class="category-list">
                                ...
                                <li v-for="(category,index) in categoryfilter" :key="index">
                                    <router-link :to="{ path: '/collection/leftsidebar/'+category.cat_name}" @click="getCategoryFilter(category.id)">
                                        {{ category.cat_name }}
                                    </router-link>
                                </li>
                            </ul>
        ...
        <div class="collection-filter-block">
            ...
            <div class="collection-collapse-block open" v-if="filterbyBrand.length">
                ...
                <b-collapse id="brand" visible accordion="myaccordion1" role="tabpanel">
                    <div class="collection-collapse-block-content">
                        <div class="collection-brand-filter">
                            <div class="custom-control custom-checkbox collection-filter-checkbox" v-for="(brand,index) in filterbyBrand" :key="index">
                                <input type="checkbox" class="custom-control-input" :value="brand.id" :id="brand.id" v-model="applyFilter" @change="appliedFilter(brand.id)" />
                                <label class="custom-control-label" v-bind:for="brand.id">{{brand.brand_display}}</label>
                            </div>
                        </div>
                    </div>
                </b-collapse>
                ...
                <b-collapse id="colors" visible accordion="myaccordion2" role="tabpanel">
                    <div class="collection-collapse-block-content">
                        <div class="collection-brand-filter color-filter">
                            <div class="custom-control custom-checkbox collection-filter-checkbox" v-for="(color,index) in filterbycolor" :key="index">
                                <input type="checkbox" class="custom-control-input" :value="color.id" :id="color.id" v-model="applyFilter" @change="appliedFilter(color.id)" />
                                <span :class="color.color_name" v-bind:style="{ 'background-color' : color.color_name}"></span>
                                <label class="custom-control-label" :class="{selected: isActive(color)}" v-bind:for="color.id">{{color.color_name}}</label>
                            </div>
                        </div>
                    </div>
                </b-collapse>
                ...
                <b-collapse id="size" visible accordion="myaccordion3" role="tabpanel">
                    <div class="collection-collapse-block-content">
                        <div class="color-selector">
                            <div class="collection-brand-filter">
                                <div class="custom-control custom-checkbox collection-filter-checkbox" v-for="(size,index) in filterbysize" :key="index">
                                    <input type="checkbox" class="custom-control-input" :value="size" :id="size" v-model="applyFilter" @change="appliedFilter(size)" />
...
export default {
    props: ["categoryfilter", "filterbyBrand", "filterbycolor"],
    data() {
        return {
    ...
    created() {
      // ""filterbyBrand", "filterbycolor"", "filterbycolor"
        console.dir(filterbyBrand)
        console.dir(filterbycolor)
    },
    components: {
        // VueSlider
    },
