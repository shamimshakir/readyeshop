<template>
	<div>
		<div class="img-wrapper">
			<div class="ribbon" v-if="product.onsale_stat"><span>On Sale</span></div>
			<div class="front">
				<router-link :to="{
				name: 'productdetails',
				params: {
				...
				id: product.id,
					...
					<img :src="getActiveImage()" :id="product.id" class="img-fluid bg-img" :alt="product.pd_name"
						:key="index" />
				</router-link>
			</div>
			<div class="cart-info cart-wrap">
				<a href="javascript:void(0)" title="Add to Wishlist" tabindex="0">
					<i class="fa fa-heart" aria-hidden="true" @click="addToWishlist(product.id)"></i>
				</a>
				<button data-toggle="modal" data-target="#addtocart" title="Add to cart" @click="addItemToCart(product.id)">
					<i class="ti-shopping-cart"></i> Add to cart
				</button>
				<a href="javascript:void(0)" title="Comapre" @click="addToCompare(product.id)">
					<i class="fa fa-refresh" aria-hidden="true"></i>
				</a>
				<a class="mobile-quick-view" title="Quick View" @click="showQuickview(product)">
