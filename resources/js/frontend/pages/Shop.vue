                            ...
                            <div class="collection-filter" :class="{ 'openFilterbar' : filter }">
                                ...
                                <div class="collection-filter-block">
                                    ...
                                    <div class="collection-collapse-block open" v-if="categories.length">
                                        ...
                                        <b-collapse id="category" visible accordion="myaccordion5" role="tabpanel">
                                            <div class="collection-collapse-block-content">
                                                <div class="collection-brand-filter">
                                                    <div class="custom-control custom-checkbox collection-filter-checkbox" v-for="(category,index) in categories" :key="index">
                                                        <input type="checkbox" class="custom-control-input" :value="category.id" :id="'category'+category.id" v-model="selectedCategories" />
                                                        <label class="custom-control-label" v-bind:for="'category'+category.id">{{category.cat_name}}</label>
                                                    </div>
                                                </div>
                                            </div>
                                        </b-collapse>
...
export default {
    ...
    watch: {
        selectedCategories: _.debounce(function () {
    ...
    methods: {
        ...
        async getCategories() {
            let dataObj = {};
            return await this.$root.getFrontData(dataObj, `/api/category_paramFilter`);
        },
        async getProducts() {
        ...
        async getShopPageInfos() {
            ...
            Promise.all([
                ...
                this.allColors = res[1].data.data;
                this.brands = res[2].data.data;
                this.allSizes = res[3].data.data;
                this.filteredProducts = res[4].data.data.data;
                this.totalColumns = res[4].data.data.total;
                this.current_page = res[4].data.data.current_page;
                this.last_page = res[4].data.data.last_page;
                this.fromItem = res[4].data.data.from;
                this.toItem = res[4].data.data.to;
                this.imageSrc = res[4].data.url;
                this.allFilters=res[5].data.data;
        ...
        async getFilteredProducts(hitPage = null) {
            ...
            this.selectedCategories.forEach((category, index) => {
                dataObj.category[index] = category;
            });
